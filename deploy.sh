#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────
# ManageLM OpenClaw Plugin — Deploy script
#
...
GITHUB_REPO="managelm/openclaw-plugin"
...
# Force required: local master → remote main (different branch names)
...
RELEASE_BODY="## ManageLM OpenClaw Plugin ${VERSION}
...
### Download
- \`${TARBALL}\` — plugin bundle (compiled plugin, skills, manifest)
